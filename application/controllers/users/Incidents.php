...
class Incidents extends CI_controller
{
    ...
    { 
      ...
      $data['incidents'] = $this->incidents_model->getIncidentByUserID($user_id);
      ...
      $data['totalIncidents'] = $this->incidents_model->getAllIncidentsNumber($user_id);

      $this->load->view('users/incidents/index',$data);
...
function add()
  ...
  $data['clients'] = $this->incidents_model->getAllClients();
...
if ($this->form_validation->run() ==true)
  ...
  { 
      ...
      if ( ! $this->upload->do_upload('filename'))
      {
              ...
              print_r($error);die;
              $this->load->view('users/incidents/add', $error);
      }
      else
      {
   ...
   $data['file'] = $this->incidents_model->add($image_name);
    ...
    redirect('users/incidents/index' ,$data);
  }
  else{
    $this->load->view('users/incidents/add', $data);
  }
}
    ...
    function edit($id)
     {
      ...
      //  die;
       ...
       $this->load->view('users/incidents/edit', $data);
    }
  ...
  { 
    ...
    //     die;
      ...
      {
              $error = array('error' => $this->upload->display_errors());
              $this->load->view('users/incidents/edit', $error);
      }
      else
      {
            $image_data = $this->upload->data();
      ...
      redirect('users/incidents/index');
      }else{
        $data['incidents_history'] = $this->incidents_model->getById_history($id);
        $data['incidents'] = $this->incidents_model->getById($id); 
       $data['company_name'] = $this->incidents_model->getAllCompanyName();
    ...
    function getAllProject()
    {
      ...
      {
         $data = $this->incidents_model->getAllProject($this->input->post('company_name'));
    ...
    function getProjectList()
    {
      ...
      if($this->input->post('company_name'))
      {
         $data = $this->incidents_model->getProjectList($this->input->post('company_name'),$project_name);
         echo json_encode ($data); die;
      }
    }
  ...
  function getAllStaff()
  {
   ...
   function incidentsView($id)
   {
    ...
    //  echo "<pre>";
      ...
      $data['company_name'] = $this->incidents_model->getAllCompanyName();
       //echo "<pre>";
      //print_r($data);die;
     $this->load->view('users/incidents/incidentsView',$data);
   }
   function OnHoldIncidents()
   {
     //$posted = json_decode($this->input->raw_input_stream);
    $user_id =$this->session->userdata('front_user');
    $incidents = $this->incidents_model->OnHoldIncidents($user_id);
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
        $html.=    "<tr><td><a href='".site_url ('front_incidentsView/' .$n->id)."'>aaaa_IN ".str_pad($n->incidents_id, '4', '0', STR_PAD_LEFT)."</a></td>
        ...
        <td>".$n->status."</td>
        <td><a class='edit' href='".site_url('front_incidents_edit/'.$n->id)."'><i class='fa fa-pencil-square-o'></i></a>&nbsp
                             <a class='delete' href='". site_url ('front_incidents_delete/'.$n->id)."' onclick='return confirm('Are you sure want to Delete this Record?')'><i class='fa fa-trash-o'></i></a>&nbsp</td>
        </tr>";
      }
      // print_r($html);die;
   ...
   {
      $user_id =$this->session->userdata('front_user');
      $incidents = $this->incidents_model->InProgressIncidents($user_id);
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
   ...
   {
     $user_id =$this->session->userdata('front_user');
      $incidents = $this->incidents_model->OpenIncidents($user_id);
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
     ...
     {
       $user_id =$this->session->userdata('front_user');
      $incidents = $this->incidents_model->CompletedIncidents( $user_id);
   ...
   {
      $user_id =$this->session->userdata('front_user');
      $incidents = $this->incidents_model->TotalIncidents($user_id);
      // print_r($incidents);die;
      $html = "";
      foreach($incidents as $n) 
      {
   ...
   function get_pdf_test($id)
    ...
    mkdir($path, 0777, true);
     ...
     $html = $this->load->view('users/incidents/incidentsPdf', $data,true);
