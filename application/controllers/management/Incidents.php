...
class Incidents extends CI_controller
{
     ...
     function index($type='') 
    {
      ...
      $data['totalIncidents'] = $this->incidents_model->getAllIncidentsNumber();
  
      $this->load->view('management/incidents/index',$data);
    ...
    function add()
    {
      ...
      if ( ! $this->upload->do_upload('filename'))
      {
       // echo "test";///die;
              ...
              print_r($error);die;
              $this->load->view('management/incidents/add', $error);
      }
      else
      {
   ...
   $data['file'] = $this->incidents_model->add($image_name);
    ...
    redirect('management/incidents/index' ,$data);
  }
  else{
    $this->load->view('management/incidents/add', $data);
  }
}
    ...
    function edit($id)
     {
      ...
      //  die;
       ...
       $this->load->view('management/incidents/edit', $data);
    }
...
$image_name = "";
  ...
  { 
    ...
    //     die;
      ...
      {
              $error = array('error' => $this->upload->display_errors());
              $this->load->view('management/incidents/edit', $error);
      }
      else
      {
            $image_data = $this->upload->data();
      ...
      redirect('management/incidents/index');
      }else{
        $data['incidents'] = $this->incidents_model->getById($id); 
       $data['company_name'] = $this->incidents_model->getAllCompanyName();
  ...
  function getAllStaff()
  {
   ...
   function incidentsView($id)
   {
     // $data['incidentss'] = $this->incidents_model->getIncidentsDetails();
      ...
      //  echo "<pre>";
      // print_r($data);die;
     $this->load->view('management/incidents/incidentsView',$data);
   }
   ...
   function OnHoldIncidents()
   {
    ...
    $incidents = $this->incidents_model->OnHoldIncidents();
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
        $html.=    "<tr><td><a href='".site_url ('admin/incidents/incidentsView/' .$n->id)."'>aaaa_IN ".str_pad($n->incidents_id, '4', '0', STR_PAD_LEFT)."</a></td>
        ...
        <td>".$n->status."</td>
        <td><a class='edit' href='".site_url('incidents_edit/'.$n->id)."'><i class='fa fa-pencil-square-o'></i></a>&nbsp
                             <a class='delete' href='". site_url ('incidents_delete/'.$n->id)."' onclick='return confirm('Are you sure want to Delete this Record?')'><i class='fa fa-trash-o'></i></a>&nbsp</td>
        </tr>";
      }
      // print_r($html);die;
   ...
   {
      $incidents = $this->incidents_model->InProgressIncidents();
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
   ...
   {
      $incidents = $this->incidents_model->OpenIncidents();
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
        ...
        <td>".$n->start_date."</td>
        <td>".date('Y-m-d',strtotime($n->end_date))."</td>
        <td>".date('H:m',strtotime($n->estimate_hours))."</td>
        <td><a class='pro-circle'><img class='img-sm' src='".site_url('assets/image/man.png')."' data-original-title='Click to deactivate the user' title=".$n->first_name."></a></td>
        <td><span class='badge badge-danger'>".$n->priority."</span></td>
        <td>".$n->status."</td>
        <td><a class='edit' href='".site_url('incidents_edit/'.$n->id)."'><i class='fa fa-pencil-square-o'></i></a>&nbsp
     ...
     {
      $incidents = $this->incidents_model->CompletedIncidents();
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n)
      {
   ...
   {
      $incidents = $this->incidents_model->TotalIncidents();
      //  print_r($data);die;
      $html = "";
      foreach($incidents as $n) 
      {
   ...
   function get_pdf_test($id)
  {
    ...
    mkdir($path, 0777, true); 
     ...
     $html = $this->load->view('management/incidents/incidentsPdf', $data,true);
