...
class Profile extends CI_Controller
{
	 function __construct()
	 {
		 ...
		 $this->load->model('management/profile_model');
	...
	function index()
	{
        ...
        if(empty($user_id)){ 
		 ...
		 if($user_id == $this->session->userdata('management')){ // admin
        ...
        //die;
	     ...
	     $this->load->view('management/profile/index',$data);
	} 
    ...
    //     die;
	   ...
	   $data['company_name'] = $this->staff_model->getAllCompanyName();
        // print_r($data);
        // die;
       $this->load->view('management/profile/edit', $data);
	}
	...
	function update($user_id)
     {
       ...
       if ($this->form_validation->run() ==true)
      {
	   ...
	   redirect('management/profile/index');
	  }else{
		...
		if($this->input->post('OldPass')!='')
		{
            ...
            {    
				if ($this->input->post('OldPass')!= $this->input->post('NewPass')){
					$user_id = $this->session->userdata('management'); 
					$stored_password = $this->login_model->get_password_management($user_id);
					
					if(password_verify($this->input->post('OldPass'),$stored_password))
					{
						//update
						$this->login_model->updatePassword_management($user_id,$this->input->post('NewPass'));
