      ...
      <div class="col-md-8">
          <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"> <a href="<?php echo site_url ('front_tasks_add');?>">Add Tasks</a></li>
            </ol>
        </nav>
      </div>
      <div class="col-md-4">
         <a href="<?php echo site_url ('front_tasks_add');?>"><button class="btn btn-outline-primary float-right"  id="addTasks"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add Tasks</button></a>
            ...
            <div class="col-md-12">
                <table id="table_id" class="display">
                    ...
                    foreach($tasks as $n) 
                    {
                        ...
                        ?>  
                        <tr>
                          <td> <a href="<?php echo site_url ('front_taskView/' .$n->id);?>"> <?php echo "aaaa_TA".str_pad($n->tasks_id, '4', '0', STR_PAD_LEFT);?></a> </td> 
                            ...
                            <td><a class="edit" href="<?php echo site_url('front_tasks_edit/'.$n->id);?>"><i class="fa fa-pencil-square-o"></i></a>&nbsp 
                            
                            <a class="delete" href="<?php echo site_url ('front_tasks_delete/'.$n->id);?>"onclick="return confirm('Are you sure want to Delete this Record?')"><i class="fa fa-trash-o"></i></a>&nbsp</td>
                        </tr>
                         <?php
                        }
                        ?>   
...
$(document).ready(function () {
  $('#InProgress').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_InProgressTasks",
      dataType: "json",
  ...
  $('#OnHold').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_OnHoldTasks",
      dataType: "json",
     success: function(response) {
       console.log(response);
       $('#table_id').dataTable().fnDestroy();
  ...
  $('#Completed').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_CompletedTasks",
      dataType: "json",
     success: function(response) {
       console.log(response);
       $('#table_id').dataTable().fnDestroy();
  ...
  $('#Open').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_OpenTasks",
      dataType: "json",
     success: function(response) {
       console.log(response);
       $('#table_id').dataTable().fnDestroy();
  ...
  $('#Total').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_TotalTasks",
      dataType: "json",
     success: function(response) {
       console.log(response);
       $('#table_id').dataTable().fnDestroy();
