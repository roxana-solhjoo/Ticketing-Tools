...
<div class="main-sec">
      ...
      <div class="col-md-8">
          <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="<?php echo site_url ('front_incidents_add');?>"> Add Incident</li>
            </ol>
        </nav>
      </div>
      <div class="col-md-4">
         <a href="<?php echo site_url ('front_incidents_add');?>"><button class="btn btn-outline-primary float-right"  id="addIncidents"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add Incidents</button></a>
  ...
  <div class="main-sec-contant">
    <div class="ProjectsDetails"> 
        ...
        <div>
          <button type="button" class="btn btn-warning btn-status" id = "OnHold">On Hold <span class="badge badge-light"><?php echo $onHoldIncidents?></span></button>
            ...
            <div class="col-md-12">
                <table id="table_id" class="display">
                    ...
                    foreach($incidents as $n) 
                    {
                        ?>
                        <tr>
                            <td><a href="<?php echo site_url ('front_incidentsView/' .$n->id);?>"> <?php echo "aaaa_IN".str_pad($n->incidents_id, '4', '0', STR_PAD_LEFT);?></a></td>
                            ...
                            <td><a class="edit" href="<?php echo site_url('front_incidents_edit/'.$n->id);?>"><i class="fa fa-pencil-square-o"></i></a>&nbsp 
                            <a class="delete" href="<?php echo site_url ('front_incidents_delete/'.$n->id);?>"onclick="return confirm('Are you sure want to Delete this Record?')"><i class="fa fa-trash-o"></i></a>&nbsp</td>
                        </tr>
                        <?php
                        }
                        ?>
...
$(document).ready(function () {
  $('#InProgress').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_InProgressIncidents",
      dataType: "json",
  ...
  $('#OnHold').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_OnHoldIncidents",
      dataType: "json",
     success: function(response) {
       console.log(response);
        $('#table_id').dataTable().fnDestroy();
  ...
  $('#Completed').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_CompletedIncidents",
      dataType: "json",
     success: function(response) {
       console.log(response);
        $('#table_id').dataTable().fnDestroy();
  ...
  $('#Open').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_OpenIncidents",
      dataType: "json",
     success: function(response) {
       console.log(response);
       $('#table_id').dataTable().fnDestroy();
  ...
  $('#Total').click(function(){
    $.ajax({
      method: "post",
      url:"<?php echo base_url(); ?>Front_TotalIncidents",
      dataType: "json",
     success: function(response) {
       console.log(response);
      $('#table_id').dataTable().fnDestroy();
